...
After the stream finishes, `handleWorkflowStream()` releases buffered chunks on the underlying workflow stream. If you manage the stream manually, call `dispose()` after you finish consuming `fullStream`.
