...
After the stream finishes, `handleChatStream()` releases buffered chunks on the underlying `MastraModelOutput`. If you manage the stream manually, call `dispose()` after you finish consuming `fullStream`.
